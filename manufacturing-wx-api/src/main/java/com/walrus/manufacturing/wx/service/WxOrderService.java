...
import com.walrus.manufacturing.core.express.ExpressService;
import com.walrus.manufacturing.core.express.dao.ExpressInfo;
import com.walrus.manufacturing.core.notify.NotifyService;
import com.walrus.manufacturing.core.notify.NotifyType;
...
import com.walrus.manufacturing.core.util.DateTimeUtil;
...
import com.walrus.manufacturing.db.util.OrderHandleOption;
import com.walrus.manufacturing.db.util.OrderUtil;
import com.walrus.manufacturing.core.util.IpUtil;
import com.walrus.manufacturing.wx.task.OrderUnpaidTask;
...
import static com.walrus.manufacturing.wx.util.WxResponseCode.*;
...
public class WxOrderService {
    ...
    @Autowired
    private ManufacturingOrderGoodsService orderGoodsService;
    @Autowired
    private ManufacturingAddressService addressService;
    @Autowired
    private ManufacturingCartService cartService;
    @Autowired
    ...
    @Autowired
    private ManufacturingCommentService commentService;
    @Autowired
    private ManufacturingCouponService couponService;
    @Autowired
    private ManufacturingCouponUserService couponUserService;
    @Autowired
    private CouponVerifyService couponVerifyService;
    @Autowired
    private TaskService taskService;
    @Autowired
    private ManufacturingAftersaleService aftersaleService;
    ...
    public Object list(Integer userId, Integer showType, Integer page, Integer limit, String sort, String order) {
        ...
        List<ManufacturingOrder> orderList = orderService.queryByOrderStatus(userId, orderStatus, page, limit, sort, order);

        List<Map<String, Object>> orderVoList = new ArrayList<>(orderList.size());
        for (ManufacturingOrder o : orderList) {
            ...
            ManufacturingGroupon groupon = grouponService.queryByOrderId(o.getId());
            if (groupon != null) {
            ...
            List<ManufacturingOrderGoods> orderGoodsList = orderGoodsService.queryByOid(o.getId());
    ...
    public Object detail(Integer userId, Integer orderId) {
        ...
        // 订单信息
        ManufacturingOrder order = orderService.findById(userId, orderId);
        if (null == order) {
        ...
        List<ManufacturingOrderGoods> orderGoodsList = orderGoodsService.queryByOid(order.getId());
    ...
    public Object submit(Integer userId, String body) {
        ...
        if (grouponRulesId != null && grouponRulesId > 0) {
            ManufacturingGrouponRules rules = grouponRulesService.findById(grouponRulesId);
            //找不到记录
            if (rules == null) {
                return ResponseUtil.badArgument();
            }
            ...
            if (grouponLinkId != null && grouponLinkId > 0) {
                ...
                // （2）不允许参加自己开团的团购
                ManufacturingGroupon groupon = grouponService.queryById(userId, grouponLinkId);
        ...
        // 收货地址
        ManufacturingAddress checkedAddress = addressService.query(userId, addressId);
        ...
        ManufacturingGrouponRules grouponRules = grouponRulesService.findById(grouponRulesId);
        if (grouponRules != null) {
            grouponPrice = grouponRules.getDiscount();
        }

        // 货品价格
        List<ManufacturingCart> checkedGoodsList = null;
        if (cartId.equals(0)) {
            checkedGoodsList = cartService.queryByUidAndChecked(userId);
        } else {
            ManufacturingCart cart = cartService.findById(cartId);
        ...
        for (ManufacturingCart checkGoods : checkedGoodsList) {
        ...
        if (couponId != 0 && couponId != -1) {
            ManufacturingCoupon coupon = couponVerifyService.checkCoupon(userId, couponId, userCouponId, checkedGoodsPrice);
            if (coupon == null) {
                return ResponseUtil.badArgumentValue();
            }
        ...
        // 添加订单商品表项
        for (ManufacturingCart cartGoods : checkedGoodsList) {
            // 订单商品
            ManufacturingOrderGoods orderGoods = new ManufacturingOrderGoods();
        ...
        for (ManufacturingCart checkGoods : checkedGoodsList) {
            ...
            ManufacturingGoodsProduct product = productService.findById(productId);
        ...
        // 如果使用了优惠券，设置优惠券使用状态
        if (couponId != 0 && couponId != -1) {
            ManufacturingCouponUser couponUser = couponUserService.findById(userCouponId);
        ...
        //如果是团购项目，添加团购信息
        if (grouponRulesId != null && grouponRulesId > 0) {
            ManufacturingGroupon groupon = new ManufacturingGroupon();
            ...
            if (grouponLinkId != null && grouponLinkId > 0) {
                //参与的团购记录
                ManufacturingGroupon baseGroupon = grouponService.queryById(grouponLinkId);
    ...
    public Object payNotify(HttpServletRequest request, HttpServletResponse response) {
        ...
        ManufacturingOrder order = orderService.findBySn(orderSn);
        if (order == null) {
        ...
        //  支付成功，有团购信息，更新团购信息
        ManufacturingGroupon groupon = grouponService.queryByOrderId(order.getId());
        if (groupon != null) {
            ManufacturingGrouponRules grouponRules = grouponRulesService.findById(groupon.getRulesId());
            ...
            List<ManufacturingGroupon> grouponList = grouponService.queryJoinRecord(groupon.getGrouponId());
            if (groupon.getGrouponId() != 0 && (grouponList.size() >= grouponRules.getDiscountMember() - 1)) {
                for (ManufacturingGroupon grouponActivity : grouponList) {
                    grouponActivity.setStatus(GrouponConstant.STATUS_SUCCEED);
                    grouponService.updateById(grouponActivity);
                }

                ManufacturingGroupon grouponSource = grouponService.queryById(groupon.getGrouponId());
    ...
    public Object goods(Integer userId, Integer orderId, Integer goodsId) {
        ...
        List<ManufacturingOrderGoods> orderGoodsList = orderGoodsService.findByOidAndGid(orderId, goodsId);
        ...
        ManufacturingOrderGoods orderGoods = orderGoodsList.get(0);
    ...
    public Object comment(Integer userId, String body) {
        ...
        ManufacturingOrderGoods orderGoods = orderGoodsService.findById(orderGoodsId);
        if (orderGoods == null) {
            return ResponseUtil.badArgumentValue();
        }
        ...
        // 1. 创建评价
        ManufacturingComment comment = new ManufacturingComment();
