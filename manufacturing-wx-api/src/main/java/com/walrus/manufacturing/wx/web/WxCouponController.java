...
import com.walrus.manufacturing.wx.vo.CouponVo;
import com.walrus.manufacturing.wx.util.WxResponseCode;
...
public class WxCouponController {
    ...
    public Object list(@RequestParam(defaultValue = "1") Integer page,
        ...
        List<ManufacturingCoupon> couponList = couponService.queryList(page, limit, sort, order);
    ...
    public Object mylist(@LoginUser Integer userId,
        ...
        List<ManufacturingCouponUser> couponUserList = couponUserService.queryList(userId, null, status, page, limit, sort, order);
        List<CouponVo> couponVoList = change(couponUserList);
        return ResponseUtil.okList(couponVoList, couponUserList);
    }

    private List<CouponVo> change(List<ManufacturingCouponUser> couponList) {
        List<CouponVo> couponVoList = new ArrayList<>(couponList.size());
        for(ManufacturingCouponUser couponUser : couponList){
            Integer couponId = couponUser.getCouponId();
            ManufacturingCoupon coupon = couponService.findById(couponId);
    ...
    public Object selectlist(@LoginUser Integer userId, Integer cartId, Integer grouponRulesId) {
        ...
        for (CouponVo cv : couponVoList) {
            ManufacturingCoupon coupon = couponVerifyService.checkCoupon(userId, cv.getCid(), cv.getId(), checkedGoodsPrice);
    ...
    public Object receive(@LoginUser Integer userId, @RequestBody String body) {
        ...
        // 用户领券记录
        ManufacturingCouponUser couponUser = new ManufacturingCouponUser();
    ...
    public Object exchange(@LoginUser Integer userId, @RequestBody String body) {
        ...
        ManufacturingCoupon coupon = couponService.findByCode(code);
        if(coupon == null){
