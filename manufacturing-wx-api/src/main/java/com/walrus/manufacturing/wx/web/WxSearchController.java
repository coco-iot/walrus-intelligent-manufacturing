...
import com.walrus.manufacturing.db.service.ManufacturingKeywordService;
import com.walrus.manufacturing.db.service.ManufacturingSearchHistoryService;
...
public class WxSearchController {
    ...
    @Autowired
    private ManufacturingKeywordService keywordsService;
    @Autowired
    ...
    public Object index(@LoginUser Integer userId) {
        //取出输入框默认的关键词
        ManufacturingKeyword defaultKeyword = keywordsService.queryDefault();
        //取出热闹关键词
        List<ManufacturingKeyword> hotKeywordList = keywordsService.queryHots();

        List<ManufacturingSearchHistory> historyList = null;
        if (userId != null) {
            //取出用户历史关键字
    ...
    public Object helper(@NotEmpty String keyword,
        ...
        List<ManufacturingKeyword> keywordsList = keywordsService.queryByKeyword(keyword, page, limit);
        String[] keys = new String[keywordsList.size()];
        int index = 0;
        for (ManufacturingKeyword key : keywordsList) {
