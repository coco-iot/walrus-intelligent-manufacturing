...
import com.walrus.manufacturing.db.service.ManufacturingNoticeAdminService;
import com.walrus.manufacturing.db.service.ManufacturingNoticeService;
...
import static com.walrus.manufacturing.admin.util.AdminResponseCode.NOTICE_UPDATE_NOT_ALLOWED;
...
public class AdminNoticeController {
    ...
    @Autowired
    private ManufacturingNoticeService noticeService;
    @Autowired
    private ManufacturingAdminService adminService;
    @Autowired
    private ManufacturingNoticeAdminService noticeAdminService;
    ...
    public Object list(String title, String content,
        ...
        List<ManufacturingNotice> noticeList = noticeService.querySelective(title, content, page, limit, sort, order);
        return ResponseUtil.okList(noticeList);
    }

    private Object validate(ManufacturingNotice notice) {
    ...
    private Integer getAdminId(){
        ...
        ManufacturingAdmin admin = (ManufacturingAdmin) currentUser.getPrincipal();
        return admin.getId();
    }
    ...
    @PostMapping("/create")
    public Object create(@RequestBody ManufacturingNotice notice) {
    ...
    public Object read(@NotNull Integer id) {
        ManufacturingNotice notice = noticeService.findById(id);
        List<ManufacturingNoticeAdmin> noticeAdminList = noticeAdminService.queryByNoticeId(id);
    ...
    @PostMapping("/update")
    public Object update(@RequestBody ManufacturingNotice notice) {
        Object error = validate(notice);
        if (error != null) {
            return error;
        }
        ManufacturingNotice originalNotice = noticeService.findById(notice.getId());
    ...
    @PostMapping("/delete")
    public Object delete(@RequestBody ManufacturingNotice notice) {
        // 1. 删除通知管理员记录
