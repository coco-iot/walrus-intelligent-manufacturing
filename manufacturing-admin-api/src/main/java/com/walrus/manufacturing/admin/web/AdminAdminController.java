...
import static com.walrus.manufacturing.admin.util.AdminResponseCode.*;
...
public class AdminAdminController {
    ...
    public Object list(String username,
        ...
        List<ManufacturingAdmin> adminList = adminService.querySelective(username, page, limit, sort, order);
        return ResponseUtil.okList(adminList);
    }

    private Object validate(ManufacturingAdmin admin) {
    ...
    @PostMapping("/create")
    public Object create(@RequestBody ManufacturingAdmin admin) {
        ...
        List<ManufacturingAdmin> adminList = adminService.findAdmin(username);
        if (adminList.size() > 0) {
    ...
    public Object read(@NotNull Integer id) {
        ManufacturingAdmin admin = adminService.findById(id);
        return ResponseUtil.ok(admin);
    }
    ...
    @PostMapping("/update")
    public Object update(@RequestBody ManufacturingAdmin admin) {
        Object error = validate(admin);
        if (error != null) {
            return error;
        }
    ...
    @PostMapping("/delete")
    public Object delete(@RequestBody ManufacturingAdmin admin) {
        Integer anotherAdminId = admin.getId();
        if (anotherAdminId == null) {
            return ResponseUtil.badArgument();
        }
        ...
        ManufacturingAdmin currentAdmin = (ManufacturingAdmin) currentUser.getPrincipal();
